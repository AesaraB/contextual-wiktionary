...
async function populateHeader(object) {
	...
	searchInput.value = searchInput.placeholder = `${humanize(object.content)}`;
	...
	if (object.params) {
		...
		if (object.params.history && object.params.history.length !== 0) {
			...
			for (const oldQuery of object.params.history) {
				...
				const oldQueryElement = populateLine({
					tag: "a",
					content: humanize(oldQuery),
					attributes: { href: "javascript:void(0)" },
					parent: historyContents,
				});
				oldQueryElement.onclick = () => define(oldQuery);
			}
		} else if (historyContents.innerHTML.trim() === "") {
			...
			if (newTimes <= 5) {
				...
				populateLine({
					tag: "span",
					content: `updated to ${currentVersion} `,
					parent: historyContents,
				});
				const patchNotes = populateLine({
					tag: "a",
					content: "(patch notes)",
					attributes: {
						href: `https://github.com/AesaraB/contextual-wiktionary/wiki/patch_notes#${currentVersion.replace(/\./g, "")}`,
					},
					parent: historyContents,
				});
				patchNotes.onclick = () => {
...
function populateDefinition(translation, parent) {
	// This function is responsible for populating the parent body.
...
function createSlider(parent) {
	const details = populateLine({
		tag: "details",
		id: "otherLangContainer",
		parent: parent,
	});
	populateLine({
		tag: "summary",
...
function transform_links(documentFragment) {
	for (const anchor of documentFragment.content.querySelectorAll("a")) {
		transformLink(anchor);
	}
}
