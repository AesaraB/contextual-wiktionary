...
async function define(query) {
	...
	if (query === null || query.match(/^ *$/)) {
		...
		await populateHeader({
			...
			params: {
				extLink: {
					title: "Open Wiktionary.org in a new tab",
	...
	await populateHeader({
		content: meta.query,
		params: { history: history, definitionPage: true },
	});
	...
	switch (true) {
		...
		case meta.hasOtherDefs: {
			...
			let otherLangParent;
			if (meta.hasEngDefs) {
				// (meta.engDefs && meta.otherLang)
				console.log(
					"popup: query has defifnitions in English and other languages",
				);
